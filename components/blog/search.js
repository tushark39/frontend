...
const Search = () => {
                ...
                {results.map((blog, i) => {
                    return (
                        ...
                        <li className="result pt-2 pb-1 pl-3" key={i} >
                            <Link href={`/blogs/${blog.slug}`}><span>
                                {blog.title}
                            </span></Link>
                        </li>
    ...
    const searchForm = () => (
        <span >
                    <input type="search" className="form-control ml-sm-2 mb-sm-1 mr-sm-2" placeholder="Search blogs" onChange={handleChange} />

                    <button className="btn btn-outline-primary ml-sm-2 my-2 my-sm-0" type="submit">
                        Search
                    </button>
        </span>
    );
